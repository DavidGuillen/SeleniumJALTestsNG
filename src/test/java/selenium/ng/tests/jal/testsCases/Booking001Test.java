...
public class Booking001Test {
	...
	private static final String AVAI_PAGE = "> AVAI_PAGE > Checks > ";
	private static final String ALPI_PAGE = "> ALPI_PAGE > Checks > ";
	...
	private static final String EXPECTED_DEPARTURE_AREA = "Paris";
	private static final String DEPARTURE_CITY_CODE = "NCE";
	private static final String DEPARTURE_CITY = "Nice";
	...
	public void normalFlow() {
		...
		// Perform Avai Checks
		defaultAvaiPageChecks();
		// Perform Avai Actions
		defaultAvaiPageActions();
		
		// Perform Alpi Checks
		defaultAlpiPageChecks();
		// Perform Alpi Actions
		defaultAlpiPageActions();
		

	}

	private void defaultAlpiPageActions() {
		AlpiPageObject.selectPassengerTitle(0, "Miss.", driver);
	}

	private void defaultAlpiPageChecks() {
		softAssert.assertEquals(AvaiPageObject.getDepartureDateAndTime(0, 0, driver), departureDateAndTime, ALPI_PAGE + "Wrong Date");
		softAssert.assertEquals(AvaiPageObject.getDepartureCity(driver), departureCity, ALPI_PAGE + "Wrong Departure City");
		softAssert.assertEquals(AvaiPageObject.getTotalPrice(driver), totalPrice, ALPI_PAGE + "Wrong Total Price");
		
	}

	private void defaultHomePageChecks() {
		softAssert.assertEquals(driver.getTitle(), EXPECTED_PAGE_TITLE, HOME_PAGE + "Wrong title");
		softAssert.assertEquals(HomePageObject.getDepartureArea(driver), "France",
				HOME_PAGE + "The departure area must be France if you connect from France");
	...
	private void defaultAvaiPageChecks() {
		departureCity = AvaiPageObject.getDepartureCity(driver);
		boolean isDepartureCityContained = false;
		if (departureCity.contains(DEPARTURE_CITY)) {
			isDepartureCityContained = true;
			System.out.println("TRUE: Esta contenido en :" + departureCity);
		}
		softAssert.assertEquals(isDepartureCityContained, true, AVAI_PAGE + "Check Departure City");
	}

	private void defaultAvaiPageActions() {
		departureDateAndTime = AvaiPageObject.getDepartureDateAndTime(0, 0, driver);
		totalPrice = AvaiPageObject.getTotalPrice(driver);
		AvaiPageObject.clickContinueButton(driver);
	}
